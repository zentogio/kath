import imageWidthsJson from './product-image-widths.json';
...
// Width variants generated by scripts/optimize-product-images.mjs, smallest
// first, largest entry is always the original file at its native width.
const imageWidths: Record<string, { width: number; file: string }[]> = imageWidthsJson;

export function productSrcset(slug: string): string {
	const variants = imageWidths[slug];
	if (!variants) return '';
	return variants.map((v) => `/products/${v.file} ${v.width}w`).join(', ');
}
